...
def get_audio_map():
    ...
    Concurrency is capped with a semaphore. Firing all requests to
    Microsoft's TTS endpoint at once (asyncio.gather with no limit)
    is not safe with edge-tts — under that much simultaneous load,
    streams can cross-talk or get truncated mid-transfer, so a
    phrase silently ends up with another phrase's audio. Capping
    it at a handful in flight at a time avoids that while still
    being much faster than doing them one at a time.
    """
    ...
    async def _gen_one(key, text, semaphore):
        async with semaphore:
            ...
            if not audio_bytes:
                raise RuntimeError(f"Empty audio for: {text}")
            return key, base64.b64encode(audio_bytes).decode()

    async def _gen_all():
        # Keyed by "roman" (each row's unique Darija romanization),
        # not by the Spanish text — two rows could share the same
        # Spanish translation and would otherwise collide onto one
        # audio clip.
        pairs = {item["roman"]: item["es"] for item in WORDS}
        semaphore = asyncio.Semaphore(MAX_CONCURRENT)
        results = await asyncio.gather(
            *[_gen_one(k, t, semaphore) for k, t in pairs.items()],
...
/* Native <audio controls>, CSS-stripped down to a bare play
   button — no scrubber, no timestamp, no download/speed menu.
   No JavaScript involved, so nothing can silently fail to fire:
   the browser's own media engine handles the click.
   Width must stay wide enough (~60px) or mobile Chrome collapses
   the whole thing into a "⋮" overflow button instead of showing
   the play icon directly. */
audio.compact-audio {
    height: 40px;
    width: 60px;
    outline: none;
...
audio.compact-audio::-webkit-media-controls-panel { background: transparent; padding: 0; }
...
audio.compact-audio::-webkit-media-controls-mute-button,
audio.compact-audio::-webkit-media-controls-volume-slider,
audio.compact-audio::-webkit-media-controls-volume-slider-container,
audio.compact-audio::-webkit-media-controls-toggle-closed-captions-button,
audio.compact-audio::-webkit-media-controls-fullscreen-button,
audio.compact-audio::-webkit-media-controls-overflow-button,
audio.compact-audio::-webkit-media-controls-overflow-menu-button {
...
for item in filtered:
    ...
    b64_audio = AUDIO_MAP.get(item["roman"])

    if b64_audio:
        # Real native <audio controls> — CSS strips it down to just
        # a round play button (see .compact-audio rules above), no
        # custom JavaScript involved at all.
        audio_html = (
            f'<audio class="compact-audio" controls preload="none" '
            f'controlsList="nodownload noplaybackrate nofullscreen">'
            f'<source src="data:audio/mpeg;base64,{b64_audio}" type="audio/mpeg">'
            f'</audio>'
        )
    else:
        audio_html = '<span style="font-size:11px;color:#a3aca7;">n/a</span>'
